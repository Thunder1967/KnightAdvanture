...
public class fly : MonoBehaviour
{
    ...
    int attack_mod; //0:idle 1:attack_go 2:attack_back
    ...
    void action()
    {
        if (attack_mod == 0)
        {
            ...
            if (Mathf.Abs(Dx_player) >= 2 && Mathf.Abs(Dx_player) <= 6 && !move_mod)
            {
                move_mod = true;
            }
            else if (Mathf.Abs(Dx_player) > 6 || Mathf.Abs(Dx_player) < 2)
            {
                move_mod = false;
            }
            ...
            if (A_C_time <= 0 && Mathf.Abs(Dx_player) <= 2)
            {
                attack_mod = 1;
        ...
        {
            rigid.velocity = new Vector2(Dx_player * Mathf.Sqrt(attackspeed / (Dx_player * Dx_player + Dy_player * Dy_player)), Dy_player * Mathf.Sqrt(attackspeed / (Dx_player * Dx_player + Dy_player * Dy_player)));
            if (Mathf.Abs(Original_Y - transform.position.y) >= Mathf.Abs(Dy_player))
            {
                if(Vector2.Distance(G_player.transform.position, transform.position) <= 1.2f)
                {
            ...
            RaycastHit2D hit_T = Physics2D.Raycast(transform.position, Vector2.up, 0.2f, 1 << 6);
            RaycastHit2D hit_B = Physics2D.Raycast(transform.position, Vector2.down, 0.45f, 1 << 6);
            RaycastHit2D hit_L = Physics2D.Raycast(transform.position, Vector2.left, 0.5f, 1 << 6);
            RaycastHit2D hit_R = Physics2D.Raycast(transform.position, Vector2.right, 0.5f, 1 << 6);
            if (hit_B.collider != null || hit_T.collider != null || hit_R.collider != null || hit_L.collider != null)
            {
                if (Vector2.Distance(G_player.transform.position, transform.position) <= 1.2f)
                {
                    player.PlayerHP -= damage;
        ...
        {
            rigid.velocity = new Vector2(-Dx_player * Mathf.Sqrt(attackspeed / (Dx_player * Dx_player + Dy_player * Dy_player)), -Dy_player * Mathf.Sqrt(attackspeed / (Dx_player * Dx_player + Dy_player * Dy_player)));
            if ((Dy_player <= 0 && transform.position.y >= Original_Y) || (Dy_player >= 0 && transform.position.y <= Original_Y))
            {
                transform.position = new Vector3(transform.position.x, Original_Y, 0);
                attack_mod = 0;
